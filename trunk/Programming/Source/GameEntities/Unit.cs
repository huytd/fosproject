...
namespace GameEntities
{
	...
	/// intellect (<see cref="GameEntities.Intellect"/>).
	/// </summary>
	public abstract class Unit : Dynamic
	{
		...
		public Unit()
		{
			...
			FilterGroups |= GameFilterGroups.UnitFilterGroup;
		}
		...
		protected override void OnDie( MapObject prejudicial )
		{
			...
			//for network games. frag counter
			if( sourceUnit != null )
			{
				if( sourceUnit.Intellect != null )
				{
					Player player = sourceUnit.Intellect.Player;
					if( player != null )
						player.Frags++;
				}
			}
			else
			{
				if( Intellect != null )
				{
					Player player = Intellect.Player;
					if( player != null )
						player.Frags--;
				}
			}
